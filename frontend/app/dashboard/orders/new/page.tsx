...
import { Search, Info, AlertCircle, Clock, ArrowRight, ArrowLeft, CheckCircle, ArrowDownToLine } from "lucide-react";
...
export default function CreateOrderPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Add file field names as placeholders
      for (const req of serviceReqs) {
        if (req.type === "file") {
          orderData[req.label] = dynamicFields[req.label] || "(uploaded)";
        }
      }
          ...
          <Card className="text-center py-12">
            <div className="relative z-10 space-y-4">
              ...
              <p className="text-sm text-white/40 max-w-md mx-auto">
                Your order has been placed and is waiting for admin acceptance. You&apos;ll see the status update in your orders.
              </p>
              <p className="text-xs text-white/25">
                Status: <span className="text-amber-400 font-semibold">WAITING ACCEPT</span>
              </p>
              <div className="flex gap-3 justify-center pt-4">
                <Button variant="outline" onClick={() => { setOrderSuccess(false); setStep(1); setSelectedService(null); setDynamicFields({}); }}>
                ...
                {serviceReqs.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {serviceReqs.map((req, idx) => {
                      if (req.type === "file") {
                        return (
                          <div key={idx} className="md:col-span-2 w-full space-y-1.5">
                            ...
                            <div className="flex items-center gap-3 p-3 rounded-xl border border-dashed border-white/[0.08] bg-white/[0.02] hover:bg-white/[0.03] transition-colors cursor-pointer">
                              <input
                                type="file"
                                accept="image/*"
                                className="hidden"
                              ...
                              <label htmlFor={`file-${idx}`} className="flex items-center gap-2 cursor-pointer w-full">
                                ...
                                <div>
                                  <p className="text-sm text-white/60">
                                    {dynamicFields[req.label] || "Click to upload image"}
                                  </p>
                                  <p className="text-[10px] text-white/25">JPG, PNG up to 5MB</p>
                                </div>
                              </label>
                            </div>
                          </div>
            ...
            <Card className="!border-blue-500/10 !bg-blue-500/[0.02]">
              <div className="flex gap-3 relative z-10">
                ...
                <div className="text-sm text-blue-300/60">
                  <p className="font-semibold text-blue-300">Balance Check</p>
                  <p className="mt-0.5">Your balance will be deducted immediately upon confirmation.</p>
                </div>
              </div>
            </Card>
          </div>
